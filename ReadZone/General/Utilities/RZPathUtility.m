...
+ (NSString *)getFileDocumentPath:(NSString *)fileName
{
	...
	NSString *documentDirectory = [RZPathUtility getDocumentPath];
...
+ (NSString *)getFileCachePath:(NSString *)fileName
{
	...
	NSString *cacheDirectory = [RZPathUtility getCachePath];
...
+ (BOOL)isExistFileInDocument:(NSString *)fileName
{
    ...
    NSString *filePath = [RZPathUtility getFileDocumentPath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
+ (BOOL)isExistFileInCache:(NSString *)fileName
{
    ...
    NSString *filePath = [RZPathUtility getFileCachePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
{
    if ([NSString isEmptyString:aFolderNameInDoc])
    {
        return YES ;
    }
    NSString *filePath = [RZPathUtility getFileDocumentPath:aFolderNameInDoc];
    if (nil == filePath)
    {
        return YES;
    }
...
{
    if ([NSString isEmptyString:aFolderNameInCahe])
    {
        return YES ;
    }
    
    if (![RZPathUtility isExistFileInCache:aFolderNameInCahe]) {
        return YES;
    }
    
    NSString *filePath = [RZPathUtility getFileCachePath:aFolderNameInCahe];
    if (nil == filePath)
    {
        return YES;
    }
...
+ (BOOL)isExistFileInResource:(NSString *)fileName
{
    ...
    NSString *filePath = [RZPathUtility getFileResourcePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
{
    if ([NSString isEmptyString:aFilePath])
    {
        return NO;
    }
    NSFileManager *fileManager = [NSFileManager defaultManager];
...
{
    if ([NSString isEmptyString:resourceName])
    {
        return NO;
    }
    //获取资源文件的存放目录进行
    NSString *resourcePath = [RZPathUtility getFileResourcePath:resourceName];
    NSString *documentPath = [RZPathUtility getFileDocumentPath:resourceName];
    ...
    if ([RZPathUtility isExistFile:documentPath])
    {
        // 如果文件已存在， 那么先删除原来的
        [RZPathUtility deleteFileAtPath:documentPath];
    }
...
+ (BOOL)createDirectoryAtDocument:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [RZPathUtility getFileDocumentPath:dirName];
...
+ (BOOL)createDirectoryAtCache:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [RZPathUtility getFileCachePath:dirName];
    if ([fileManager fileExistsAtPath:dirPath])
    {
        return YES;
    }
...
+ (BOOL)createDirectoryAtTemporary:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *tempPath = [RZPathUtility getTemporaryPath];
...
+ (NSString *)reCorrentPathWithPath:(NSString *)path
{
    ...
    NSString *docPath = [RZPathUtility getDocumentPath];
    ...
    if (range.length <= 0)
    {
        ...
        if (docRange.length > 0)
        {
            ...
            NSString *newPath = [RZPathUtility getFileDocumentPath:relPath];
            return newPath;
        }
    }
    return path;
...
{
    return [RZPathUtility getTemporaryPath];
}
